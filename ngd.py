        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                if whitening_matrices_list_values and not len(d_p.shape) == 1:
                    ...
                    print('NGD: psi_tensor max = {}, min = {}'.format(np.max(psi.flatten()), np.min(psi.flatten())))
                    print('NGD: gamma_tensor max = {}, min = {}'.format(np.max(gamma.flatten()), np.min(gamma.flatten())))
